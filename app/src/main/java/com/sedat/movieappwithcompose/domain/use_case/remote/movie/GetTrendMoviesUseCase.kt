...
class GetTrendMoviesUseCase @Inject constructor(
    private val movieRepository: MovieRepository
) {
    operator fun invoke(time: String, region: String, page: Int, language: String): Flow<Resource<List<Movie>>> = flow{
        try {
            emit(Resource.loading(null))

            val data = movieRepository.getTrendMovies(time = time, region = region, page = page, language = language)

            if(data.results.isNotEmpty())
                emit(Resource.success(data.toMovie()))
            else
                emit(Resource.error("Trend Movies Not Found!", null))
        }catch (e: Exception){
